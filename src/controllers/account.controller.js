...
exports.postLoginSocialNetwork = async (req, res) => {
  try {
    const { email, name, avt, id, type } = req.user;

    // Nếu Facebook không trả về email, tạo email giả để vẫn tạo account
    const userEmail = email || `${type}_${id}@noemail.com`;

    // Tìm hoặc tạo account theo email
    let account = await AccountModel.findOne({ email: userEmail });
    if (!account) {
      account = await AccountModel.create({
        email: userEmail,
        authType: type,
        password: '' // Đăng nhập MXH không cần mật khẩu
      });
    }

    // Format lại tên người dùng
    const formattedName = typeof name === 'object'
      ? `${name.givenName || ''} ${name.familyName || ''}`.trim()
      : name;

    // Tìm hoặc tạo user tương ứng
    let user = await UserModel.findOne({ accountId: account._id });
    if (!user) {
    ...
    console.error('postLoginSocialNetwork ERROR: ', error);
